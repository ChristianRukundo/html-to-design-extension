...
export class Builder {
    constructor(api, capture, assetBytes, options, callbacks = {}) {
        this.api = api;
        ...
        this.xNudges = new Map();
        this.layers = 0;
        this.sinceYield = 0;
        this.lookupImage = (ref) => this.imageHashes.get(ref) ?? null;
    }
    async build() {
        ...
        for (const root of this.capture.roots) {
            const frame = (await this.buildNode(root, null));
            if (!frame)
                continue;
    ...
    async prepareFonts() {
        ...
        if (missing.length > 0) {
            this.warnings.push({
                code: 'font.missing',
                message: `Not installed in Figma, substituted: ${missing.slice(0, 8).join(', ')}${missing.length > 8 ? `, +${missing.length - 8} more` : ''}`,
            });
        }
    }
    prepareImages() {
        ...
        for (const [ref, asset] of Object.entries(this.capture.assets)) {
            ...
            try {
                ...
                this.warnings.push({
                    code: 'image.rejected',
                    message: `Figma rejected an image: ${error.message}`,
                });
            }
    ...
    async buildNode(node, parent) {
        ...
        if (node.opacity < 1 && 'opacity' in built)
            built.opacity = node.opacity;
        if (node.blendMode !== 'NORMAL' && 'blendMode' in built)
            built.blendMode = node.blendMode;
    ...
    buildText(node) {
        ...
        try {
            ...
            this.warnings.push({
                code: 'text.failed',
                message: `Could not create a text layer: ${error.message}`,
                nodeName: node.name,
            });
            text.remove();
            return null;
     ...
     */
    fitToCapture(text, node, lineHeight) {
        ...
        if (delta > 0) {
            ...
            if (node.align === 'CENTER')
                this.xNudges.set(text, delta / 2);
            else if (node.align === 'RIGHT')
                this.xNudges.set(text, delta);
        }
    }
    /**
    ...
    buildImage(node) {
        // The capture walker can classify an <img src="*.svg"> as an IMAGE node
        // while retaining the asset as SVG markup. Treat that case as a vector so
        // logos and other external SVG images are not silently dropped.
        const asset = this.capture.assets[node.asset];
        if (asset?.kind === 'SVG') {
            return this.buildSvg({ ...node, kind: 'SVG' });
        }
    ...
    buildSvg(node) {
        ...
        try {
            ...
            this.warnings.push({
                code: 'svg.failed',
                message: `Could not import an SVG: ${error.message}`,
                nodeName: node.name,
            });
            return null;
        }
     ...
     */
    applyLayout(frame, node) {
        const layout = node.layout;
        if (layout.mode === 'ABSOLUTE' || !this.options.autoLayout)
            return;
        ...
        try {
            ...
            this.revertLayout(frame, node, `Figma rejected the layout: ${error.message}`);
            return;
        }
    ...
    revertLayout(frame, node, reason) {
...
// Helpers
// ---------------------------------------------------------------------------
function applyChildSizing(frame, sources) {
    frame.children.forEach((child, index) => {
...
function setSizing(child, property, mode) {
    if (mode === 'FIXED')
        return;
...
function measureDrift(frame, sources, widthOverrides) {
    let worst = 0;
    frame.children.forEach((child, index) => {
        const source = sources[index];
        if (!source)
            return;
        worst = Math.max(worst, Math.abs(child.x - source.rect.x), Math.abs(child.y - source.rect.y), Math.abs(child.width - (widthOverrides.get(child) ?? source.rect.width)), Math.abs(child.height - source.rect.height));
    });
    return worst;
}
/** The pixel height of one line, from whatever unit the capture recorded. */
function lineHeightPx(lineHeight, fontSize) {
    if (lineHeight.unit === 'PIXELS')
        return Math.max(1, lineHeight.value);
    if (lineHeight.unit === 'PERCENT')
        return Math.max(1, (fontSize * lineHeight.value) / 100);
    return fontSize * 1.2;
}
function applyStroke(node, stroke) {
    ...
    node.strokeWeight = Math.max(stroke.weights.top, stroke.weights.right, stroke.weights.bottom, stroke.weights.left);
...
function applyCorners(node, corners) {
