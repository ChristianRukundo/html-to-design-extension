...
import { isGradient, parseGradient, parsePosition } from './gradient.js';
/**
 ...
 */
export function parseBackground(style, box, assets) {
    const paints = [];
    const unsupported = [];
    ...
    if (image && image !== 'none') {
        ...
        for (let i = layers.length - 1; i >= 0; i--) {
            ...
            const paint = parseLayer(layer, box, assets, cyclic(sizes, i), cyclic(positions, i), cyclic(repeats, i));
            if (paint)
                paints.push(paint);
            else if (layer !== 'none')
                unsupported.push(layer.slice(0, 40));
        }
    }
    return { paints, unsupported };
}
function parseLayer(layer, box, assets, size, position, repeat) {
...
function backgroundScaleMode(size, repeat) {
    const normalizedRepeat = repeat.trim().toLowerCase();
    const normalizedSize = size.trim().toLowerCase();
    if (normalizedSize === 'cover')
        return 'FILL';
 ...
 */
function cropGeometry(size, position, repeat, box) {
    ...
    if (width === null || width <= 0 || box.width <= 0)
