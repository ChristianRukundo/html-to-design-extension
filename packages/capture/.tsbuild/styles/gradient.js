 ...
 */
export function parseGradient(value, box) {
...
// linear-gradient
// ---------------------------------------------------------------------------
function parseLinear(args, box, repeating) {
    ...
    const lineLength = Math.abs(box.width * Math.sin(rad)) + Math.abs(box.height * Math.cos(rad)) || 1;
 ...
 * corners.
 */
function sideOrCornerToAngle(spec, box) {
...
// radial-gradient
// ---------------------------------------------------------------------------
function parseRadial(args, box, repeating) {
    const parts = splitTopLevel(args);
    if (parts.length === 0)
        return null;
    ...
    if (head && isRadialConfig(head)) {
        ...
        if (positionPart)
            center = parsePosition(positionPart, box);
 ...
 */
function isRadialConfig(part) {
    const tokens = splitWhitespace(part).map((t) => t.toLowerCase());
    if (tokens.length === 0)
        return false;
    if (tokens.includes('at'))
        return true;
    if (tokens.includes('circle') || tokens.includes('ellipse'))
        return true;
    if (tokens.some((t) => SIZE_KEYWORDS.has(t)))
        return true;
    // An explicit radius such as `radial-gradient(60px 30px, …)`.
    return tokens.every((t) => /^[-+.\d]/.test(t));
}
function resolveRadialSize(keyword, isCircle, center, box) {
    const w = box.width || 1;
    const h = box.height || 1;
    const cx = center.x * w;
    const cy = center.y * h;
...
// conic-gradient
// ---------------------------------------------------------------------------
function parseConic(args, box, repeating) {
    const parts = splitTopLevel(args);
    if (parts.length === 0)
        return null;
    let angle = 0;
...
function parseStops(items, lineLength, repeating, angular = false) {
    const raw = [];
...
function normalizePositions(stops) {
    ...
    while (i < stops.length) {
        ...
        const start = i - 1;
        let end = i;
        while (end < stops.length && stops[end].position === null)
            end++;
 ...
 */
function unrollRepeating(stops) {
    ...
    for (let rep = -before; rep <= after; rep++) {
        ...
        for (const stop of stops) {
            ...
            if (position < -0.001 || position > 1.001)
                continue;
...
export function parsePosition(value, box) {
...
const COLOR_START = /^(#|rgb|rgba|hsl|hsla|hwb|lab|lch|oklab|oklch|color\(|color-mix|transparent|currentcolor|[a-z]{3,})/i;
function looksLikeColorStop(item) {
