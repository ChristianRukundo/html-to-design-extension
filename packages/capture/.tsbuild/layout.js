 ...
 */
export function inferLayout(style, children, parent, autoLayoutEnabled) {
...
function pickAxis(style, children) {
...
function inferAxisFromGeometry(children) {
...
function groupIntoLines(children, axis) {
    if (axis === 'VERTICAL') {
...
// Single-line layouts
// ---------------------------------------------------------------------------
function singleLine(style, children, parent, axis) {
    const horizontal = axis === 'HORIZONTAL';
    const ordered = [...children].sort((a, b) => horizontal ? a.rect.x - b.rect.x : a.rect.y - b.rect.y);
    const primaryStart = (c) => (horizontal ? c.rect.x : c.rect.y);
    const primaryEnd = (c) => horizontal ? c.rect.x + c.rect.width : c.rect.y + c.rect.height;
    const counterStart = (c) => (horizontal ? c.rect.y : c.rect.x);
    const counterEnd = (c) => horizontal ? c.rect.y + c.rect.height : c.rect.x + c.rect.width;
    const gaps = [];
    for (let i = 1; i < ordered.length; i++) {
        gaps.push(primaryStart(ordered[i]) - primaryEnd(ordered[i - 1]));
    }
    // Overlapping children cannot be a flow layout.
    if (gaps.some((g) => g < -EPSILON))
        return ABSOLUTE_RESULT(children.length);
    const uniformGap = uniform(gaps);
    const spaceBetween = style.justifyContent === 'space-between' && ordered.length > 1;
    if (uniformGap === null && !spaceBetween)
        return ABSOLUTE_RESULT(children.length);
    ...
    const primaryAlign = spaceBetween ? 'SPACE_BETWEEN' : 'MIN';
    return {
        layout: {
            mode: axis,
 ...
 */
function analyzeCounterAxis(ordered, parent, start, end, horizontal) {
    const extent = horizontal ? parent.height : parent.width;
    const starts = ordered.map(start);
    const ends = ordered.map(end);
    const minStart = Math.min(...starts);
    const maxEnd = Math.max(...ends);
    if (minStart < -EPSILON || maxEnd > extent + EPSILON)
        return null;
    const padStart = Math.max(0, minStart);
    const padEnd = Math.max(0, extent - maxEnd);
    // All children span the full content box: stretch.
    const allFill = ordered.every((c, i) => Math.abs(starts[i] - padStart) < EPSILON && Math.abs(ends[i] - (extent - padEnd)) < EPSILON);
    if (allFill)
        return { align: 'MIN', padStart, padEnd, fill: true };
    ...
    if (centred)
        return { align: 'CENTER', padStart, padEnd, fill: false };
    return null;
}
function orderedSizingBackToInput(children, ordered, horizontal, counterFill) {
    return children.map((child) => {
        const growsOnPrimary = child.grow > 0 && ordered.includes(child);
        const primary = growsOnPrimary ? 'FILL' : 'FIXED';
        const counter = counterFill && child.stretches ? 'FILL' : 'FIXED';
        return horizontal
...
// Wrapping layouts
// ---------------------------------------------------------------------------
function wrapped(style, children, rows, parent) {
