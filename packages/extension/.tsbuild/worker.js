...
import { blockedReason, captureFilename, CancelledError, countLayers, groupWarnings, runCapture, } from './capture-run.js';
import { ViewportEmulator } from './viewport.js';
import { CHUNK_SIZE, DEFAULT_SETTINGS, } from './protocol.js';
/**
...
chrome.runtime.onConnect.addListener((port) => {
    ...
    port.onMessage.addListener((message) => void handlePopupMessage(message, port));
    ...
    try {
        ...
        try {
            for (const width of widths) {
                ...
                if (width > 0)
                    await emulator.setWidth(width);
                captures.push(await runCapture({
                    tabId: tab.id,
        ...
        setState(error instanceof CancelledError
            ? { status: 'idle' }
 ...
 */
async function save(json, filename, compress) {
