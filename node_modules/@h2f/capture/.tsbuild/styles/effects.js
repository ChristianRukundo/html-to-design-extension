 ...
 */
export function parseEffects(style) {
    const effects = [];
...
// ---------------------------------------------------------------------------
function parseFilterList(value, effects, unsupported, blurKind) {
    if (!value || value === 'none')
        return;
    for (const item of splitWhitespace(value)) {
        ...
        switch (fn.name) {
            case 'blur': {
                ...
                if (radius > 0)
                    effects.push({ kind: blurKind, radius: radius * 2 });
                break;
            }
            case 'drop-shadow': {
