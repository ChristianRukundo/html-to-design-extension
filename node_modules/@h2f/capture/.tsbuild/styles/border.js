 ...
 */
export function parseBorder(style) {
    ...
    for (const side of SIDES) {
        ...
        if (isTransparent(color))
            continue;
        weights[side.toLowerCase()] = width;
...
function dashPatternFor(lineStyle, width) {
    switch (lineStyle) {
        case 'dashed':
 ...
 */
export function parseCorners(style, width, height) {
    let elliptical = false;
    const read = (property, reference) => {
 ...
 */
function clampCorners(corners, width, height) {
