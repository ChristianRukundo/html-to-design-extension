 ...
 */
const IDENTITY = { kind: 'NONE', rotation: 0, scaleX: 1, scaleY: 1 };
export function analyzeTransform(value) {
    if (!value || value === 'none')
        return IDENTITY;
 ...
 * rotation · scale · skew.
 */
function decompose([a0, b0, c0, d0]) {
    let a = a0;
    let b = b0;
    let c = c0;
    let d = d0;
    ...
    const isIdentity = Math.abs(rotation) < 1e-4 && Math.abs(scaleX - 1) < 1e-4 && Math.abs(scaleY - 1) < 1e-4;
    return {
