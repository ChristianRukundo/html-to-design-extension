...
export function nameFor(element, textSample) {
    ...
    if (id && !isGenerated(id))
        return truncate(`${tag}#${id}`);
    const semantic = semanticClass(element);
    if (semantic)
        return truncate(`${tag}.${semantic}`);
    const role = element.getAttribute('role');
    if (role)
        return truncate(`${tag}[${role}]`);
    return tag;
}
function semanticClass(element) {
    ...
    for (const candidate of className.trim().split(/\s+/)) {
        ...
        if (isGenerated(candidate))
            continue;
        if (UTILITY_CLASS.test(candidate))
            continue;
        return candidate;
    }
    return null;
...
function isGenerated(value) {
    if (/__[a-z0-9]{5,}$/i.test(value))
        return true;
    if (/^(?:css|sc|jsx)-[a-z0-9]{5,}$/i.test(value))
        return true;
    if (/^[a-z]?[0-9a-f]{7,}$/i.test(value))
        return true;
    // Mostly digits and letters with no word structure at all.
    if (value.length > 8 && !/[aeiou]/i.test(value))
        return true;
    return false;
}
