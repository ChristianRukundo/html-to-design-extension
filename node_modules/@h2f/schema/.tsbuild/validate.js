...
export function validateCapture(input) {
    ...
    if (doc.version !== SCHEMA_VERSION) {
        errors.push(`unsupported schema version ${String(doc.version)}; this plugin reads version ${SCHEMA_VERSION}`);
...
function walk(node, path, errors, assets, seenIds) {
    ...
    switch (node.kind) {
        case 'ROOT':
        case 'ELEMENT': {
            if (node.rasterize !== undefined)
                requireAsset(node.rasterize, here, errors, assets);
        ...
        case 'TEXT': {
            ...
            for (const seg of node.segments ?? []) {
                if (seg.start < 0 || seg.end > node.characters.length || seg.start >= seg.end) {
                    errors.push(`${here}: segment [${seg.start}, ${seg.end}) is out of range for ${node.characters.length} characters`);
                }
            }
            break;
        }
