 ...
 */
export function cropRegion(element, image, scale) {
    if (!(scale > 0) || image.width <= 0 || image.height <= 0)
        return null;
    ...
    const clipped = left < -0.5 || top < -0.5 || right > image.width + 0.5 || bottom > image.height + 0.5;
