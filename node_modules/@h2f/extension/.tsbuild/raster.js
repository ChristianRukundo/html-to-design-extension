...
export async function rasterizeMarked(capture, options) {
    const warnings = [];
    const ids = (await listRasterTargets(options.tabId)).filter((id) => `raster:${id}` in capture.assets);
    ...
    for (const [index, id] of targets.entries()) {
        ...
        try {
            ...
            if (!target)
                throw new Error('the element disappeared while capturing');
            ...
            try {
                ...
                if (!region)
                    throw new Error('the element was not visible after scrolling to it');
                if (region.clipped) {
                    warnings.push({
                        code: 'raster.clipped',
                ...
                context.drawImage(shot, region.sx, region.sy, region.sWidth, region.sHeight, 0, 0, region.width, region.height);
...
async function captureVisibleTab(windowId) {
    try {
        ...
        if (!/per second/i.test(error.message))
            throw error;
        await delay(1000);
