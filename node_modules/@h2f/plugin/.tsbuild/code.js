...
async function runImport() {
    ...
    try {
        if (current.pageName) {
            // Page creation is available in Figma Desktop and browser, but keep the
            // import usable in restricted/plugin-test hosts that expose only the
            // current page. In that case the capture still imports successfully.
            try {
                const page = figma.createPage();
                page.name = current.pageName;
                figma.currentPage = page;
            }
            catch {
                // Continue on the current page rather than losing the whole batch.
            }
        }
 ...
 */
function placeBesideExistingContent(roots) {
    const others = figma.currentPage.children.filter((child) => !roots.includes(child));
    if (others.length === 0)
        return;
